...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'senpai' | 'senpai-pixel':
				dialogue = CoolUtil.coolTextFile(Paths.txt('senpai/senpaiDialogue'));
			case 'roses' | 'roses-pixel':
				dialogue = CoolUtil.coolTextFile(Paths.txt('roses/rosesDialogue'));
			case 'thorns' | 'thorns-pixel':
				dialogue = CoolUtil.coolTextFile(Paths.txt('thorns/thornsDialogue'));
		}

		switch(SONG.song.toLowerCase())
		{
			case 'spookeez' | 'monster' | 'south' | 'no-iphone': 
			{
				curStage = 'spooky';
				halloweenLevel = true;
			...
			case 'pico' | 'blammed' | 'philly' | 'bad-pico': 
					{
					curStage = 'philly';
			...
			case 'senpai' | 'roses' | 'senpai-pixel' | 'roses-pixel':
			{
					curStage = 'school';
					...
					if (SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'roses-pixel')
						{
							bgGirls.getScared();
					}
		...
		if (SONG.song.toLowerCase().endsWith('-pixel'))
			gfVersion = 'gf-pixel';
		if (SONG.song.toLowerCase() == 'no-iphone')
			gfVersion = 'gf-iphone';
		if (SONG.song.toLowerCase() == 'bad-pico')
			gfVersion = 'gf-bad';
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - Clay Mod v1");
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var senpaiEvilpixel:FlxSprite = new FlxSprite();
		senpaiEvilpixel.frames = Paths.getSparrowAtlas('weeb/senpaiCrazy_pixel');
		senpaiEvilpixel.animation.addByPrefix('idle', 'Senpai Pre Explosion', 24, false);
		senpaiEvilpixel.setGraphicSize(Std.int(senpaiEvilpixel.width * 6));
		senpaiEvilpixel.scrollFactor.set();
		senpaiEvilpixel.updateHitbox();
		senpaiEvilpixel.screenCenter();

		if (SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'thorns' || SONG.song.toLowerCase() == 'thorns-pixel')
		{
			remove(black);

			if (SONG.song.toLowerCase() == 'thorns' || SONG.song.toLowerCase() == 'thorns-pixel')
			{
				add(red);
			}
		}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					else if (SONG.song.toLowerCase() == 'thorns-pixel')
						{
							add(senpaiEvilpixel);
							senpaiEvilpixel.alpha = 0;
							new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
							{
								senpaiEvilpixel.alpha += 0.15;
								if (senpaiEvilpixel.alpha < 1)
								{
									swagTimer.reset();
								}
								else
								{
									senpaiEvilpixel.animation.play('idle');
									FlxG.sound.play(Paths.sound('Senpai_Dies'), 1, false, null, true, function()
									{
										remove(senpaiEvilpixel);
										remove(red);
										FlxG.camera.fade(FlxColor.WHITE, 0.01, true, function()
										{
											add(dialogueBox);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					switch (SONG.song.toLowerCase())
					{
					case 'senpai-pixel' | 'roses-pixel' | 'thorns-pixel':
					sustainNote.x = 100;
					}
				...
				if (swagNote.mustPress)
				{
					...
					{
					case 'senpai-pixel' | 'roses-pixel' | 'thorns-pixel':
					swagNote.x += 100 / 10; // general offset
					}
				else
				{
				}
		...
		for (i in 0...4)
		{
			...
			switch (SONG.song.toLowerCase())
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					babyArrow.frames = Paths.getSparrowAtlas('weeb/NOTE_assets_week6');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
				...
				case 'senpai-pixel' | 'roses-pixel' | 'thorns-pixel':
					babyArrow.loadGraphic(Paths.image('weeb/arrow_pixel'), true, 17, 17);
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
			...
			if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && !up && !down && !right && !left)
			{
				...
				playerStrums.forEach(function(spr:FlxSprite)
				{
					...
					if (spr.animation.curAnim.name == 'confirm' && !SONG.song.toLowerCase().endsWith('-pixel'))
					{
						spr.centerOffsets();
						spr.offset.x -= 13;
						spr.offset.y -= 13;
